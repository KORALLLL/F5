...
PRETRAINED_VOCAB_PATH = files("f5_tts").joinpath("../../data/RUSLAN/vocab.txt")
...
MAX_WORKERS = 16  # Leave one CPU free
...
def batch_convert_texts(texts, polyphone, batch_size=BATCH_SIZE):
    # char-level: ничего не конвертируем, просто возвращаем исходный текст
    return texts
    ...
    with open(csv_file_path, mode="r", newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            if len(row) >= 2:
                ...
                audio_file_path = (parent / Path("RUSLAN") / audio_file).with_suffix(".wav")
